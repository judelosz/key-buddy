...
Scope in progress: **Phase 4 — curriculum vertical slice + post-MVP shell.** Phases 0–3 are implemented. The post-MVP tab shell (Missions · Free Play · AFK Mode · Progress · Settings) is in; landing onboarding, the curriculum engine (modules/lessons/exercises/tier gates), Tier 1–5 content, SongMastery foundation, and the validated XP/Level meter are being built now. Streaks and Riffs are deferred from the new UI (data preserved); SessionBuilder/adaptive difficulty (Phase 5), AFK games (Phase 6), full curriculum expansion, and mic input remain future work.
