...
**Authoring status (2026-07-28): Tiers 6–7 are authored** (`m6-twelve-bar`, `m7-dominant-color`; gates 6–7 live; walkPath proves both completable). Deliberate deltas from the table, recorded honestly: (a) the Tier-6 "identify the next change from audio" gate item is approximated by a major-vs-dom7 chord-ear lesson plus mid-form number-retrieval quiz questions — a true next-change ear generator is a TODO; (b) "three choruses" is implemented as the mastery-star chorus plus `repeatedSessions: 2` distinct-day evidence on the core skill rather than three back-to-back choruses (continuity-based multi-chorus evidence should arrive with the Tier-10 boss work); (c) Tier-7's "build and play three dom7s" is the required `l-dc-build` mission (all of C7/F7/G7) feeding the gate's core-skill mastery, and the 80% ear bar lives in the tier checkpoint. Tier 8 remains blocked on the swing-ratio grading spec below. *(2026-08-03: unblocked and authored — `m8-shuffle-feel`, gate 8, Frankie and Johnny; the spec is `docs/09-swing-grading-spec.md`. Deltas: (d) the "reference recording" listen lesson uses the app's own swung arrangement, not an external recording — recording-anchored lessons stay a G11 TODO; (e) the minor-pentatonic call-and-response gate item is deferred to the Tier-10 boss work (G8 improvise-an-answer) — Tier 8's gate stands on the swung boss + ratio bar + checkpoint. **Tier 9 authored the same day** — `m9-smooth-changes`, gate 9, C.C. Rider (inversion comping over steady roots); its "three choruses with ≥85% correct chord onsets" is implemented as the mastery-star chorus + `maxStops: 0` continuity bars on apply/boss + `repeatedSessions: 2` distinct-day evidence, and the "hear I→IV / V→I" ear item is approximated by a major/dom7 refresher + checkpoint questions until the next-change ear generator exists.)*
...
- **Swing is graded on a ratio band, not a triplet grid** (G7): score the offbeat within a tolerance band around a tempo-appropriate swing ratio (~1.7:1–2.5:1 at Tier-8 tempos), with per-player bias correction; report a measured "your swing ratio" and its drift (flattening-toward-straight is the #1 documented failure mode). A grading spec must exist before Tier-8 content is authored — the widened symmetric tap windows would let a player who never swings pass. *(2026-08-03: spec'd and implemented — see `docs/09-swing-grading-spec.md`: swung expected-onset transform, ratio-band measurement with pair-differencing bias cancellation, `Attempt.swing` readout + flattening drift, declared `minSwingInBandPct` bars, and a swing-aware mastery star on swung charts.)*
